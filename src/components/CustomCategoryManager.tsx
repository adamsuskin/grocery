...
export function CustomCategoryManager({ listId, onClose, permissionLevel, onViewStatistics }: CustomCategoryManagerProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        if (hasActiveFilters) {
          clearFilters();
        } else if (!deletingId && !showBulkDeleteConfirm && !showMergeDialog && !showCopyModal && !showBackupRestore) {
          onClose();
        }
      }
    };
  ...
  }, [onClose, deletingId, showBulkDeleteConfirm, showMergeDialog, showCopyModal, showBackupRestore, hasActiveFilters, clearFilters]);
                ...
                {canEdit && (
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      ...
                      title="Backup and restore categories"
                    >
